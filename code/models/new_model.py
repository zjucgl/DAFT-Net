...
class FusionBModel(nn.Module):
    def __init__(self):
        super(FusionBModel, self).__init__()
        self.num_heads = 8
        self.embed_dim = 768
        self.seq_len = 2
        ...
        # 定义层归一化层
        self.raw_data_norm = nn.LayerNorm(self.raw_data_dim)
        self.basic_data_norm = nn.LayerNorm(self.basic_data_dim)
        self.blood_data_norm = nn.LayerNorm(self.blood_data_dim)
        ...
        self.output_fc = nn.Linear(self.embed_dim * 4, 1)  # 拼接后有 4 个部分，所以这里是 embed_dim * 4
        self.sigmoid = nn.Sigmoid()

    def forward(self, basic_data, blood_data, text_data_encoded, raw_data, basic_risk_data_encoded, blood_risk_data_encoded):
        # 对输入数据进行归一化
        raw_data = self.raw_data_norm(raw_data)
        basic_data = self.basic_data_norm(basic_data)
        blood_data = self.blood_data_norm(blood_data)
